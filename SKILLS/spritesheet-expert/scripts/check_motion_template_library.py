...
def main() -> int:
    ...
    for template_id, template in templates.items():
        ...
        if template.get("status") != "approved":
            item_errors.append(f"template status is {template.get('status')!r}, not 'approved'")
        ...
        results.append({"template_id": template_id, "ok": not item_errors, "errors": item_errors})
        errors.extend(f"{template_id}: {message}" for message in item_errors)

    report = {"ok": not errors, "templates": results, "errors": errors}
